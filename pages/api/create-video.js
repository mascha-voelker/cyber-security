...
export default async function handler(req, res) {
  ...
  try {
    const { script, userStrategies } = req.body; // CHANGED: userStrategies instead of username
    
    if (!script) {
      return res.status(400).json({ error: 'Script is required' });
    }

    // ADDED: Validation for userStrategies
    if (!userStrategies) {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        userStrategies: userStrategies  // CHANGED: from username to userStrategies
      });
